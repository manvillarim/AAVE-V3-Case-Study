...
definition CouplingInv() returns bool =
    ...
    a.lastClaimReturn == ao.lastClaimReturn &&
    a.lastClaimAllList.length == ao.lastClaimAllList.length &&
    (forall uint256 i.
        i < a.lastClaimAllList.length =>
        a.lastClaimAllList[i] == ao.lastClaimAllList[i]
    ) &&
    a.lastClaimAllAmounts.length == ao.lastClaimAllAmounts.length &&
    (forall uint256 i.
...
rule gasOptimizedCorrectnessOfClaimRewardsInstr(method f, method g)
filtered {
    f -> f.selector == sig:a.claimRewards_instr(address[], uint256, address, address).selector,
    g -> g.selector == sig:ao.claimRewards_instr(address[], uint256, address, address).selector
} {
    gasOptimizationCorrectness(f, g);
}

rule gasOptimizedCorrectnessOfClaimRewardsOnBehalfInstr(method f, method g)
filtered {
    f -> f.selector == sig:a.claimRewardsOnBehalf_instr(address[], uint256, address, address, address).selector,
    g -> g.selector == sig:ao.claimRewardsOnBehalf_instr(address[], uint256, address, address, address).selector
} {
    gasOptimizationCorrectness(f, g);
}

rule gasOptimizedCorrectnessOfClaimRewardsToSelfInstr(method f, method g)
filtered {
    f -> f.selector == sig:a.claimRewardsToSelf_instr(address[], uint256, address).selector,
    g -> g.selector == sig:ao.claimRewardsToSelf_instr(address[], uint256, address).selector
} {
    gasOptimizationCorrectness(f, g);
}

rule gasOptimizedCorrectnessOfClaimAllRewardsInstr(method f, method g)
filtered {
    f -> f.selector == sig:a.claimAllRewards_instr(address[], address).selector,
    g -> g.selector == sig:ao.claimAllRewards_instr(address[], address).selector
} {
    gasOptimizationCorrectness(f, g);
}

rule gasOptimizedCorrectnessOfClaimAllRewardsOnBehalfInstr(method f, method g)
filtered {
    f -> f.selector == sig:a.claimAllRewardsOnBehalf_instr(address[], address, address).selector,
    g -> g.selector == sig:ao.claimAllRewardsOnBehalf_instr(address[], address, address).selector
} {
    gasOptimizationCorrectness(f, g);
}

rule gasOptimizedCorrectnessOfClaimAllRewardsToSelfInstr(method f, method g)
filtered {
    f -> f.selector == sig:a.claimAllRewardsToSelf_instr(address[]).selector,
    g -> g.selector == sig:ao.claimAllRewardsToSelf_instr(address[]).selector
} {
    gasOptimizationCorrectness(f, g);
}