...
rule gasOptimizedCorrectnessOfDeltaOfInstr(method f, method g)
filtered {
    f -> f.selector == sig:a.deltaOf_instr(uint256).selector,
    g -> g.selector == sig:ao.deltaOf_instr(uint256).selector
} {
    gasOptimizationCorrectness(f, g);
}

rule gasOptimizedCorrectnessOfBalanceOfInstr(method f, method g)
filtered {
    f -> f.selector == sig:a.balanceOf_instr(uint256, address).selector,
    g -> g.selector == sig:ao.balanceOf_instr(uint256, address).selector
} {
    gasOptimizationCorrectness(f, g);
}