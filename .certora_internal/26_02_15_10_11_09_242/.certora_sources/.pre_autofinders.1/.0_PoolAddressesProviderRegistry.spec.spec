...
definition couplingInv() returns bool =
    ...
    // NOVO: Injetividade - se provider tem índice válido, lista[índice] = provider
    (forall address provider.
        a._addressesProvidersIndexes[provider] < a._addressesProvidersList.length =>
        a._addressesProvidersList[a._addressesProvidersIndexes[provider]] == provider
    ) &&
    
    // NOVO: Providers registrados têm índice válido
    (forall address provider.
        a._addressesProviderToId[provider] != 0 =>
        a._addressesProvidersIndexes[provider] < a._addressesProvidersList.length
    ) &&
    
    // NOVO: Providers não registrados não estão na lista ativa (índice >= length ou índice 0 especial)
    (forall address provider.
        a._addressesProviderToId[provider] == 0 =>
        (a._addressesProvidersIndexes[provider] >= a._addressesProvidersList.length ||
         (a._addressesProvidersIndexes[provider] == 0 && 
          (a._addressesProvidersList.length == 0 || a._addressesProvidersList[0] != provider)))
    ) &&
    
    // NOVO: Lista não tem address(0)
    (forall uint256 i.
        i < a._addressesProvidersList.length =>
        a._addressesProvidersList[i] != 0
    );
