...
contract PoolAddressesProviderRegistry is Ownable, IPoolAddressesProviderRegistry {
  ...
  function getAddressesProvidersList() external view override returns (address[] memory) {
    ...
    // RULE 25 - Cache array length in loops (for memory array)
  ...
  function registerAddressesProvider(address provider, uint256 id) external override onlyOwner {
    ...
    // RULE 27 - Use mappings instead of arrays for data lists (inline _addToAddressesProvidersList)
    // RULE 5 - Cache storage variables
    uint256 currentSize = _addressesProvidersListSize;assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff00000004,currentSize)}
    _addressesProvidersIndexes[provider] = currentSize;
    _addressesProvidersList[currentSize] = provider;
    unchecked { 
      _addressesProvidersListSize++; // Preserves overflow behaviour of push()
    }
  ...
  function unregisterAddressesProvider(address provider) external override onlyOwner {
    // RULE 5 - Cache storage variables
    uint256 oldId = _addressesProviderToId[provider];assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff00000005,oldId)}
    ...
    // RULE 27 - Use mappings instead of arrays for data lists (inline _removeFromAddressesProvidersList)
