...
definition couplingInv() returns bool =
    ...
    (forall uint256 streamId.
        // Quotient by the instance address: `createStream` stores `address(this)`
        // in `sender`, so the two instances legitimately differ there. We relate
        // the fields either by equality or under the renaming addr(a) |-> addr(ao)
        // that any single-address deployment collapses to the identity.
        (ghost_a_streams_sender[streamId] == ghost_ao_streams_sender[streamId] ||
         (ghost_a_streams_sender[streamId] == a && ghost_ao_streams_sender[streamId] == ao)) &&
