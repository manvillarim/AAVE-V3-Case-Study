...
contract RewardsDistributorOptimized is RewardsDistributor {
    ...
    function getAllUserRewards_instr(address[] calldata assets, address user) external returns (address[] memory, uint256[] memory) {
        (address[] memory rl, uint256[] memory ua) = this.getAllUserRewards(assets, user);assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff00010002,0)}
    ...
    function getAssetIndex_instr(address asset, address reward) external returns (uint256, uint256) {
        (uint256 n, uint256 o) = this.getAssetIndex(asset, reward);assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff00010003,0)}
        lastAssetIndexNew = n;
        lastAssetIndexOld = o;
        return (n, o);
    }
    ...
    function _getUserAssetBalances(
        ...
        userAssetBalances = new RewardsDataTypes.UserAssetBalance[](assets.length);assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff00020004,0)}
        for (uint256 i = 0; i < assets.length; i++) {
            userAssetBalances[i].asset = assets[i];address certora_local5 = userAssetBalances[i].asset;assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff00000005,certora_local5)}
            userAssetBalances[i].userBalance = IScaledBalanceToken(assets[i]).scaledBalanceOf(user);uint256 certora_local6 = userAssetBalances[i].userBalance;assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff00000006,certora_local6)}
            userAssetBalances[i].totalSupply = IScaledBalanceToken(assets[i]).scaledTotalSupply();uint256 certora_local7 = userAssetBalances[i].totalSupply;assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff00000007,certora_local7)}
        }
