...
definition CouplingInv() returns bool =
    ...
    a.lastGetUserRewardsReturn == ao.lastGetUserRewardsReturn &&
    a.lastAssetIndexNew == ao.lastAssetIndexNew &&
    a.lastAssetIndexOld == ao.lastAssetIndexOld &&

    a.lastAllRewardsList.length == ao.lastAllRewardsList.length &&
    (forall uint256 i.
        i < a.lastAllRewardsList.length =>
        a.lastAllRewardsList[i] == ao.lastAllRewardsList[i]
    ) &&
    a.lastAllUnclaimedAmounts.length == ao.lastAllUnclaimedAmounts.length &&
    (forall uint256 i.
...
rule gasOptimizedCorrectnessOfGetUserRewardsInstr(method f, method g)
filtered {
    f -> f.selector == sig:a.getUserRewards_instr(address[], address, address).selector,
    g -> g.selector == sig:ao.getUserRewards_instr(address[], address, address).selector
} {
    gasOptimizationCorrectness(f, g);
}

rule gasOptimizedCorrectnessOfGetAssetIndexInstr(method f, method g)
filtered {
    f -> f.selector == sig:a.getAssetIndex_instr(address, address).selector,
    g -> g.selector == sig:ao.getAssetIndex_instr(address, address).selector
} {
    gasOptimizationCorrectness(f, g);
}

rule gasOptimizedCorrectnessOfGetAllUserRewardsInstr(method f, method g)
filtered {
    f -> f.selector == sig:a.getAllUserRewards_instr(address[], address).selector,
    g -> g.selector == sig:ao.getAllUserRewards_instr(address[], address).selector
} {
    gasOptimizationCorrectness(f, g);
}
