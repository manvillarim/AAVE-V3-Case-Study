import "Structures/GhostRegistry.spec";
...
definition couplingInv() returns bool =
    ...
    a._owner == ao._owner &&
    // Array length (via ghost) equivalent to mapping size counter
    ghostArrayLength == ao._addressesProvidersListSize &&
    // Array elements (via ghost) equivalent to mapping entries within valid range
    (forall uint256 i.
        i < ghostArrayLength =>
        ghostArrayElements[i] == ao._addressesProvidersList[i]
    ) &&
    // Entries beyond the active range in the optimized mapping are zeroed out
    (forall uint256 i.
        i >= ao._addressesProvidersListSize =>
        ao._addressesProvidersList[i] == 0
    ) &&
