...
contract RewardsController is RewardsDistributor, VersionedInitializable, IRewardsController {
  ...
  function configureAssets(
    ...
    uint256 configLength = config.length;assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff0000000a,configLength)}
    // RULE 9  - Avoid explicit zero initialization (i declared without = 0)
    for (uint256 i; i < configLength; ) {
      config[i].totalSupply = IScaledBalanceToken(config[i].asset).scaledTotalSupply();uint256 certora_local25 = config[i].totalSupply;assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff00000019,certora_local25)}
      _installTransferStrategy(config[i].reward, config[i].transferStrategy);
      _setRewardOracle(config[i].reward, config[i].rewardOracle);
      // RULE 26 - Pre-increment; RULE 28 - Unchecked arithmetic: i < configLength guarantees no overflow
      unchecked { ++i; }
    }
  ...
  function _getUserAssetBalances(
    ...
    // RULE 25 - Cache array length in loops
    uint256 assetsLength = assets.length;assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff0000000b,assetsLength)}
    userAssetBalances = new RewardsDataTypes.UserAssetBalance[](assetsLength);assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff00020014,0)}
    ...
    for (uint256 i; i < assetsLength; ) {
      ...
      userAssetBalances[i].asset = asset;address certora_local26 = userAssetBalances[i].asset;assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff0000001a,certora_local26)}
      (userAssetBalances[i].userBalance, userAssetBalances[i].totalSupply) = IScaledBalanceToken(
        asset
      ).getScaledUserBalanceAndSupply(user);assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff0002001b,0)}
      // RULE 26 - Pre-increment; RULE 28 - Unchecked arithmetic: i < assetsLength guarantees no overflow
      unchecked { ++i; }
    }
  }
  ...
  function _claimRewards(
    ...
    if (amount != 0) {
      ...
      for (uint256 i; i < assetsLength; ) {
        // RULE 24 - Cache array member variable
        address asset = assets[i];assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff0000001c,asset)}
        totalRewards += _assets[asset].rewards[reward].usersData[user].accrued;assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff0000001e,totalRewards)}

        if (totalRewards <= amount) {
          _assets[asset].rewards[reward].usersData[user].accrued = 0;
        } else {
          uint256 difference = totalRewards - amount;assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff0000001f,difference)}
          totalRewards -= difference;assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff00000020,totalRewards)}
  ...
  function _claimAllRewards(
    ...
    uint256 assetsLength = assets.length;assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff0000000d,assetsLength)}
    ...
    for (uint256 i; i < assetsLength; ) {
      // RULE 24 - Cache array member variable
      address asset = assets[i];assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff00000018,asset)}
      for (uint256 j; j < rewardsListLength; ) {
        if (rewardsList[j] == address(0)) {
          rewardsList[j] = _rewardsList[j];
        }
        uint256 rewardAmount = _assets[asset].rewards[rewardsList[j]].usersData[user].accrued;assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff0000001d,rewardAmount)}
        if (rewardAmount != 0) {
          claimedAmounts[j] += rewardAmount;
          _assets[asset].rewards[rewardsList[j]].usersData[user].accrued = 0;
        }
        // RULE 26 - Pre-increment; RULE 28 - Unchecked arithmetic: j < rewardsListLength guarantees no overflow
        unchecked { ++j; }
      }
      // RULE 26 - Pre-increment; RULE 28 - Unchecked arithmetic: i < assetsLength guarantees no overflow
      unchecked { ++i; }
    ...
    for (uint256 i; i < rewardsListLength; ) {
      ...
      // RULE 26 - Pre-increment; RULE 28 - Unchecked arithmetic: i < rewardsListLength guarantees no overflow
      unchecked { ++i; }
    }
  }
