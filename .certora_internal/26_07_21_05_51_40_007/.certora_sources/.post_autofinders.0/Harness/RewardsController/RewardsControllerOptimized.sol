...
constructor(address emissionManager) RewardsController(emissionManager) {}
    ...
    function claimRewards_instr(address[] calldata assets, uint256 amount, address to, address reward) external returns (uint256) {
        ...
        uint256 r = _claimRewards(assets, amount, msg.sender, msg.sender, to, reward);assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff00000001,r)}
        lastClaimReturn = r;
        return r;
    }
    ...
    function claimRewardsOnBehalf_instr(address[] calldata assets, uint256 amount, address user, address to, address reward) external onlyAuthorizedClaimers(msg.sender, user) returns (uint256) {
        ...
        require(to != address(0), 'INVALID_TO_ADDRESS');
        uint256 r = _claimRewards(assets, amount, msg.sender, user, to, reward);assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff00000002,r)}
        lastClaimReturn = r;
        return r;
    }

    function claimRewardsToSelf_instr(address[] calldata assets, uint256 amount, address reward) external returns (uint256) {
        uint256 r = _claimRewards(assets, amount, msg.sender, msg.sender, msg.sender, reward);assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff00000003,r)}
        lastClaimReturn = r;
        return r;
    }
    ...
    function claimAllRewards_instr(address[] calldata assets, address to) external returns (address[] memory, uint256[] memory) {
        require(to != address(0), 'INVALID_TO_ADDRESS');
        (address[] memory rl, uint256[] memory ca) = _claimAllRewards(assets, msg.sender, msg.sender, to);assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff00010004,0)}
        lastClaimAllList = rl;
    ...
    function claimAllRewardsOnBehalf_instr(address[] calldata assets, address user, address to) external onlyAuthorizedClaimers(msg.sender, user) returns (address[] memory, uint256[] memory) {
        require(user != address(0), 'INVALID_USER_ADDRESS');
        require(to != address(0), 'INVALID_TO_ADDRESS');
        (address[] memory rl, uint256[] memory ca) = _claimAllRewards(assets, msg.sender, user, to);assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff00010005,0)}
        lastClaimAllList = rl;
        lastClaimAllAmounts = ca;
        return (rl, ca);
    }
    ...
    function claimAllRewardsToSelf_instr(address[] calldata assets) external returns (address[] memory, uint256[] memory) {
        (address[] memory rl, uint256[] memory ca) = _claimAllRewards(assets, msg.sender, msg.sender, msg.sender);assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff00010006,0)}
        lastClaimAllList = rl;
        lastClaimAllAmounts = ca;
        return (rl, ca);
    }
