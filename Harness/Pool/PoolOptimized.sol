...
import {PoolInstance} from "aave-v3-origin-optimized/src/contracts/instances/PoolInstance.sol";
import {IPoolAddressesProvider} from "aave-v3-origin-optimized/src/contracts/interfaces/IPoolAddressesProvider.sol";
import {DataTypes} from "aave-v3-origin-optimized/src/contracts/protocol/libraries/types/DataTypes.sol";

contract PoolOptimized is PoolInstance {
    ...
    function getReserveData_instr(address asset) external {
        DataTypes.ReserveDataLegacy memory r = this.getReserveData(asset);
    ...
    function getEModeCategoryData_instr(uint8 id) external {
        DataTypes.EModeCategoryLegacy memory c = this.getEModeCategoryData(id);
        emLtv = c.ltv;
    ...
    function getEModeCategoryCollateralConfig_instr(uint8 id) external {
        DataTypes.CollateralConfig memory c = this.getEModeCategoryCollateralConfig(id);
        ccLtv = c.ltv;
