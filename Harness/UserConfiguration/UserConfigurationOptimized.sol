...
import {UserConfiguration} from "aave-v3-origin-optimized/src/contracts/protocol/libraries/configuration/UserConfiguration.sol";
import {DataTypes} from "aave-v3-origin-optimized/src/contracts/protocol/libraries/types/DataTypes.sol";

contract UserConfigurationOptimized {
    ...
    function setBorrowingCached_instr(address user, uint256 reserveIndex, bool borrowing) external {
        ...
        cache.setBorrowingInMemory(reserveIndex, borrowing);
    ...
    function setUsingAsCollateralCached_instr(address user, uint256 reserveIndex, bool usingAsCollateral) external {
        DataTypes.UserConfigurationMap memory cache = _usersConfig[user];
        cache.setUsingAsCollateralInMemory(reserveIndex, usingAsCollateral);
        _usersConfig[user].data = cache.data;
    }
