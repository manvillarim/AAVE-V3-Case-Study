...
`registerAddressesProvider` shows a reduction of 229 gas (avg) in our variant relative to both Original and Cyfrin. The metered calls never take the revert path, so this is not the cost of the revert itself: the three guards name their messages through `Errors` constants (`string public constant`), which the compiler materialises in memory on every call. Substituting the inline literals `'8'`, `'8'` and `'86'` for those constants in the Cyfrin variant, while keeping the `require` statements, already reproduces our figures exactly (118,192 gas here and 44,104 in `unregisterAddressesProvider`). The saving is therefore attributable to dropping the constant dereference rather than to the custom error, which on its own is worth only a few gas per guard.

`unregisterAddressesProvider` saves 56 gas (avg) over Cyfrin and 153 gas (avg) over the original, by the same mechanism and with one guard instead of three.
